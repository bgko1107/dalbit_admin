...
<div class="widget widget-table" id="div_yearScroll">
    <div class="widget-content">
        ...
        <div id="div_yearStateSimpleTable">
            <table class="table table-bordered _tableHeight" data-height="23px" id="simpleDayTable">
                ...
                <tbody id="tableYearStateBody_simple"></tbody>
            </table>
        </div>

        <div id="div_yearStateDetailTable" class="col-md-12 no-padding" style="display: none">
            <table class="table table-bordered _tableHeight yearStateTable" data-height="23px" style="width: 3500px;">
                <colgroup>
                ...
                <tr>
                    ...
                    <th colspan="3" class="_bgColor _fontColor" data-bgColor="#4472c4" data-fontcolor="white">마이스타</th>
                    <th class="_bgColor _fontColor" data-bgColor="#4472c4" data-fontcolor="white">선물</th>
                    <th class="_bgColor _fontColor" data-bgColor="#4472c4" data-fontcolor="white">팬 등록</th>
                    <th colspan="4" class="_bgColor _fontColor" data-bgColor="#4472c4" data-fontcolor="white">팬 보드</th>
                ...
                <tr>
                    ...
                    <th colspan="2" class="_bgColor" data-bgColor="#b4c7e7">신규 댓글 등록</th>
                ...
                <tbody id="tableYearStateBody_detail"></tbody>
            </table>
        </div>
    </div>
...
<div class="modal fade" id="yearStateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
<script type="text/javascript">
    ...
    function fn_yearState_simple_success(data, response){
        ...
        if(!isDataEmpty){
            var template = $('#tmp_tableYearStateBody_simpleTotal').html();
            var templateScript = Handlebars.compile(template);
            var totalContext = response.data.totalInfo;
            var totalHtml = templateScript(totalContext);
            $("#tableYearStateBody_simple").append(totalHtml);

            response.data.detailList.slctType = slctType;
        }

        var template = $('#tableYearStateBody_simpleDetail').html();
        var templateScript = Handlebars.compile(template);
        ...
        $("#tableYearStateBody_simple").append(html);

        if(isDataEmpty){
            $("#tableYearStateBody_simple td:last").remove();
        }else{
            $("#tableYearStateBody_simple").append(totalHtml);
        }
    ...
    function fn_yearState_detail_success(data, response){
        ...
        if(!isDataEmpty){
            var template = $('#tmp_tableYearStateBody_detailTotal').html();
            var templateScript = Handlebars.compile(template);
            var totalContext = response.data.totalInfo;
            var totalHtml = templateScript(totalContext);
            $("#tableYearStateBody_detail").append(totalHtml);

            response.data.detailList.slctType = slctType;
        }

        var template = $('#tableYearStateBody_detailDetail').html();
        var templateScript = Handlebars.compile(template);
        response.data.detailList.nowMonth = Number(moment().format("MM"));
        response.data.detailList.nowDay = Number(moment().format("DD"));
        response.data.detailList.nowHour = Number(moment().format("HH"));
        ...
        var detailContext = response.data.detailList;
        var html=templateScript(detailContext);
        $("#tableYearStateBody_detail").append(html);

        if(isDataEmpty){
            $("#tableYearStateBody_detail td:last").remove();
        }else{
            $("#tableYearStateBody_detail").append(totalHtml);
        }
        ...
        monthRowspan("detailYear");
        totalRowspan("detailYearTotal");
    }

    function yearTableSwich(){
        $("#div_yearStateDetailTable").hide();
        $("#div_yearStateSimpleTable").hide();
        if( $("#div_yearScroll").find("#bt_tableSw").html() == "전체보기"){
            $("#div_yearScroll").css('width','3500px');
            $("#div_yearScroll").find("#bt_tableSw").html('간략하게');
            $("#div_yearScroll").find("#div_yearStateDetailTable").show();
        }else{
            $("#div_yearScroll").css('width','');
            $("#div_yearScroll").find("#bt_tableSw").html('전체보기');
            $("#div_yearScroll").find("#div_yearStateSimpleTable").show();
        }
    }
    ...
    function fn_yearState_detail_popup_success(dst_id, response, param){
        $('#yearStateModal').modal("show");
        $("#yearStateModal").find("#type").html(util.getCommonCodeLabel(param.type,status_detail_push_type));
        $("#yearStateModal").find("#date").html(param.startDate);
        $("#yearStateModal").find("#sendCount").html('<span style="color:blue;">' + response.data.totalInfo.sendmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.sendfCnt + '</span> / ' + response.data.totalInfo.sendnCnt);
        $("#yearStateModal").find("#succCount").html('<span style="color:blue;">' + response.data.totalInfo.succmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.succfCnt + '</span> / ' + response.data.totalInfo.succnCnt);
        $("#yearStateModal").find("#failCount").html('<span style="color:blue;">' + response.data.totalInfo.failmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.failfCnt + '</span> / ' + response.data.totalInfo.failnCnt);

    }

    function fn_yearState_simple_popup_success(dst_id, response, param){
        $('#yearStateModal').modal("show");
        $("#yearStateModal").find("#type").html(util.getCommonCodeLabel(param.type,status_simple_push_type));
        $("#yearStateModal").find("#date").html(param.startDate);
        $("#yearStateModal").find("#sendCount").html('<span style="color:blue;">' + response.data.totalInfo.sendmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.sendfCnt + '</span> / ' + response.data.totalInfo.sendnCnt);
        $("#yearStateModal").find("#succCount").html('<span style="color:blue;">' + response.data.totalInfo.succmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.succfCnt + '</span> / ' + response.data.totalInfo.succnCnt);
        $("#yearStateModal").find("#failCount").html('<span style="color:blue;">' + response.data.totalInfo.failmCnt + '</span> / <span style="color:red;">' + response.data.totalInfo.failfCnt + '</span> / ' + response.data.totalInfo.failnCnt);
...
<script type="text/x-handlebars-template" id="tableYearStateBody_simpleDetail">
    {{#each this as |data|}}
    <tr {{#dalbit_if ../nowDay '==' data.day}} class="font-bold _stateSubTh" {{/dalbit_if}} >
        ...
        <td class="yearTotal _bgColor font-bold" data-bgcolor="#bfbfbf" data-year="{{data.the_date}}">
...
<script type="text/x-handlebars-template" id="tmp_tableYearStateBody_detailTotal">
    <tr class="font-bold">
        <td class="_bgColor" data-bgcolor="#bfbfbf">총합</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">성공수치</span><br/>(수신거부)<br/>(거부비율%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p11Succ}}</span><br/>({{addComma p11Fail}})<br/>({{average p11mFail p11Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p18Succ}}</span><br/>({{addComma p18Fail}})<br/>({{average p18mFail p18Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p30Succ}}</span><br/>({{addComma p30Fail}})<br/>({{average p30mFail p30Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p15Succ}}</span><br/>({{addComma p15Fail}})<br/>({{average p15mFail p15Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p16Succ}}</span><br/>({{addComma p16Fail}})<br/>({{average p16mFail p16Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p17Succ}}</span><br/>({{addComma p17Fail}})<br/>({{average p17mFail p17Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p21Succ}}</span><br/>({{addComma p21Fail}})<br/>({{average p21mFail p21Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p22Succ}}</span><br/>({{addComma p22Fail}})<br/>({{average p22mFail p22Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p23Succ}}</span><br/>({{addComma p23Fail}})<br/>({{average p23mFail p23Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p31Succ}}</span><br/>({{addComma p31Fail}})<br/>({{average p31mFail p31Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p32Succ}}</span><br/>({{addComma p32Fail}})<br/>({{average p32mFail p32Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p33Succ}}</span><br/>({{addComma p33Fail}})<br/>({{average p33mFail p33Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p20Succ}}</span><br/>({{addComma p20Fail}})<br/>({{average p20mFail p20Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p19Succ}}</span><br/>({{addComma p19Fail}})<br/>({{average p19mFail p19Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p51Succ}}</span><br/>({{addComma p51Fail}})<br/>({{average p51mFail p51Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p52Succ}}</span><br/>({{addComma p52Fail}})<br/>({{average p52mFail p52Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p60Succ}}</span><br/>({{addComma p60Fail}})<br/>({{average p60mFail p60Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p62Succ}}</span><br/>({{addComma p62Fail}})<br/>({{average p62mFail p62Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p63Succ}}</span><br/>({{addComma p63Fail}})<br/>({{average p63mFail p63Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p53Succ}}</span><br/>({{addComma p53Fail}})<br/>({{average p53mFail p53Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p50Succ}}</span><br/>({{addComma p50Fail}})<br/>({{average p50mFail p50Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p54Succ}}</span><br/>({{addComma p54Fail}})<br/>({{average p54mFail p54Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p55Succ}}</span><br/>({{addComma p55Fail}})<br/>({{average p55mFail p55Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p56Succ}}</span><br/>({{addComma p56Fail}})<br/>({{average p56mFail p56Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p57Succ}}</span><br/>({{addComma p57Fail}})<br/>({{average p57mFail p57Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p58Succ}}</span><br/>({{addComma p58Fail}})<br/>({{average p58mFail p58Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p61Succ}}</span><br/>({{addComma p61Fail}})<br/>({{average p61mFail p61Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p92Succ}}</span><br/>({{addComma p92Fail}})<br/>({{average p92mFail p92Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p93Succ}}</span><br/>({{addComma p93Fail}})<br/>({{average p93mFail p93Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p94Succ}}</span><br/>({{addComma p94Fail}})<br/>({{average p94mFail p94Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p95Succ}}</span><br/>({{addComma p95Fail}})<br/>({{average p95mFail p95Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma p96Succ}}</span><br/>({{addComma p96Fail}})<br/>({{average p96mFail p96Succ}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma totSucc}}</span><br/>({{addComma totFail}})<br/>({{average totmFail totSucc}}%)</td>
        <td class="_bgColor" data-bgcolor="#f2f2f2"><span class="_fontColor" data-fontcolor="#ca6927">{{addComma totSucc}}</span><br/>({{addComma totFail}})<br/>({{average totmFail totSucc}}%)</td>
    </tr>
</script>

<script type="text/x-handlebars-template" id="tableYearStateBody_detailDetail">
    {{#each this as |data|}}
    <tr {{#dalbit_if ../nowDay '==' data.day}} class="font-bold _stateSubTh"  {{/dalbit_if}}>
    <td class="detailYear _bgColor font-bold" data-bgcolor="#b4c7e7">{{data.the_date}}</td>
    <td class="_fontColor _bgColor" data-bgColor="#dae3f3" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{{sexIcon data.gb}}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="11" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p11Succ '!=' 0}}<span class="font-bold">{{addComma p11Succ}}</span><br/>({{addComma p11Fail}})<br/>({{average p11Fail p11Succ}}%){{else}}{{#dalbit_if p11Fail '!=' 0}}<span class="font-bold">{{addComma p11Succ}}</span><br/>({{addComma p11Fail}})<br/>({{average p11Fail p11Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="18" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p18Succ '!=' 0}}<span class="font-bold">{{addComma p18Succ}}</span><br/>({{addComma p18Fail}})<br/>({{average p18Fail p18Succ}}%){{else}}{{#dalbit_if p18Fail '!=' 0}}<span class="font-bold">{{addComma p18Succ}}</span><br/>({{addComma p18Fail}})<br/>({{average p18Fail p18Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="30" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p30Succ '!=' 0}}<span class="font-bold">{{addComma p30Succ}}</span><br/>({{addComma p30Fail}})<br/>({{average p30Fail p30Succ}}%){{else}}{{#dalbit_if p30Fail '!=' 0}}<span class="font-bold">{{addComma p30Succ}}</span><br/>({{addComma p30Fail}})<br/>({{average p30Fail p30Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="15" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p15Succ '!=' 0}}<span class="font-bold">{{addComma p15Succ}}</span><br/>({{addComma p15Fail}})<br/>({{average p15Fail p15Succ}}%){{else}}{{#dalbit_if p15Fail '!=' 0}}<span class="font-bold">{{addComma p15Succ}}</span><br/>({{addComma p15Fail}})<br/>({{average p15Fail p15Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="16" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p16Succ '!=' 0}}<span class="font-bold">{{addComma p16Succ}}</span><br/>({{addComma p16Fail}})<br/>({{average p16Fail p16Succ}}%){{else}}{{#dalbit_if p16Fail '!=' 0}}<span class="font-bold">{{addComma p16Succ}}</span><br/>({{addComma p16Fail}})<br/>({{average p16Fail p16Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="17" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p17Succ '!=' 0}}<span class="font-bold">{{addComma p17Succ}}</span><br/>({{addComma p17Fail}})<br/>({{average p17Fail p17Succ}}%){{else}}{{#dalbit_if p17Fail '!=' 0}}<span class="font-bold">{{addComma p17Succ}}</span><br/>({{addComma p17Fail}})<br/>({{average p17Fail p17Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="21" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p21Succ '!=' 0}}<span class="font-bold">{{addComma p21Succ}}</span><br/>({{addComma p21Fail}})<br/>({{average p21Fail p21Succ}}%){{else}}{{#dalbit_if p21Fail '!=' 0}}<span class="font-bold">{{addComma p21Succ}}</span><br/>({{addComma p21Fail}})<br/>({{average p21Fail p21Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="22" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p22Succ '!=' 0}}<span class="font-bold">{{addComma p22Succ}}</span><br/>({{addComma p22Fail}})<br/>({{average p22Fail p22Succ}}%){{else}}{{#dalbit_if p22Fail '!=' 0}}<span class="font-bold">{{addComma p22Succ}}</span><br/>({{addComma p22Fail}})<br/>({{average p22Fail p22Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="23" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p23Succ '!=' 0}}<span class="font-bold">{{addComma p23Succ}}</span><br/>({{addComma p23Fail}})<br/>({{average p23Fail p23Succ}}%){{else}}{{#dalbit_if p23Fail '!=' 0}}<span class="font-bold">{{addComma p23Succ}}</span><br/>({{addComma p23Fail}})<br/>({{average p23Fail p23Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="31" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p31Succ '!=' 0}}<span class="font-bold">{{addComma p31Succ}}</span><br/>({{addComma p31Fail}})<br/>({{average p31Fail p31Succ}}%){{else}}{{#dalbit_if p31Fail '!=' 0}}<span class="font-bold">{{addComma p31Succ}}</span><br/>({{addComma p31Fail}})<br/>({{average p31Fail p31Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="32" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p32Succ '!=' 0}}<span class="font-bold">{{addComma p32Succ}}</span><br/>({{addComma p32Fail}})<br/>({{average p32Fail p32Succ}}%){{else}}{{#dalbit_if p32Fail '!=' 0}}<span class="font-bold">{{addComma p32Succ}}</span><br/>({{addComma p32Fail}})<br/>({{average p32Fail p32Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="33" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p33Succ '!=' 0}}<span class="font-bold">{{addComma p33Succ}}</span><br/>({{addComma p33Fail}})<br/>({{average p33Fail p33Succ}}%){{else}}{{#dalbit_if p33Fail '!=' 0}}<span class="font-bold">{{addComma p33Succ}}</span><br/>({{addComma p33Fail}})<br/>({{average p33Fail p33Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="20" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p20Succ '!=' 0}}<span class="font-bold">{{addComma p20Succ}}</span><br/>({{addComma p20Fail}})<br/>({{average p20Fail p20Succ}}%){{else}}{{#dalbit_if p20Fail '!=' 0}}<span class="font-bold">{{addComma p20Succ}}</span><br/>({{addComma p20Fail}})<br/>({{average p20Fail p20Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="19" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p19Succ '!=' 0}}<span class="font-bold">{{addComma p19Succ}}</span><br/>({{addComma p19Fail}})<br/>({{average p19Fail p19Succ}}%){{else}}{{#dalbit_if p19Fail '!=' 0}}<span class="font-bold">{{addComma p19Succ}}</span><br/>({{addComma p19Fail}})<br/>({{average p19Fail p19Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="51" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p51Succ '!=' 0}}<span class="font-bold">{{addComma p51Succ}}</span><br/>({{addComma p51Fail}})<br/>({{average p51Fail p51Succ}}%){{else}}{{#dalbit_if p51Fail '!=' 0}}<span class="font-bold">{{addComma p51Succ}}</span><br/>({{addComma p51Fail}})<br/>({{average p51Fail p51Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="52" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p52Succ '!=' 0}}<span class="font-bold">{{addComma p52Succ}}</span><br/>({{addComma p52Fail}})<br/>({{average p52Fail p52Succ}}%){{else}}{{#dalbit_if p52Fail '!=' 0}}<span class="font-bold">{{addComma p52Succ}}</span><br/>({{addComma p52Fail}})<br/>({{average p52Fail p52Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="60" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p60Succ '!=' 0}}<span class="font-bold">{{addComma p60Succ}}</span><br/>({{addComma p60Fail}})<br/>({{average p60Fail p60Succ}}%){{else}}{{#dalbit_if p60Fail '!=' 0}}<span class="font-bold">{{addComma p60Succ}}</span><br/>({{addComma p60Fail}})<br/>({{average p60Fail p60Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="62" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p62Succ '!=' 0}}<span class="font-bold">{{addComma p62Succ}}</span><br/>({{addComma p62Fail}})<br/>({{average p62Fail p62Succ}}%){{else}}{{#dalbit_if p62Fail '!=' 0}}<span class="font-bold">{{addComma p62Succ}}</span><br/>({{addComma p62Fail}})<br/>({{average p62Fail p62Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="63" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p63Succ '!=' 0}}<span class="font-bold">{{addComma p63Succ}}</span><br/>({{addComma p63Fail}})<br/>({{average p63Fail p63Succ}}%){{else}}{{#dalbit_if p63Fail '!=' 0}}<span class="font-bold">{{addComma p63Succ}}</span><br/>({{addComma p63Fail}})<br/>({{average p63Fail p63Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="53" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p53Succ '!=' 0}}<span class="font-bold">{{addComma p53Succ}}</span><br/>({{addComma p53Fail}})<br/>({{average p53Fail p53Succ}}%){{else}}{{#dalbit_if p53Fail '!=' 0}}<span class="font-bold">{{addComma p53Succ}}</span><br/>({{addComma p53Fail}})<br/>({{average p53Fail p53Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="50" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p50Succ '!=' 0}}<span class="font-bold">{{addComma p50Succ}}</span><br/>({{addComma p50Fail}})<br/>({{average p50Fail p50Succ}}%){{else}}{{#dalbit_if p50Fail '!=' 0}}<span class="font-bold">{{addComma p50Succ}}</span><br/>({{addComma p50Fail}})<br/>({{average p50Fail p50Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="54" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p54Succ '!=' 0}}<span class="font-bold">{{addComma p54Succ}}</span><br/>({{addComma p54Fail}})<br/>({{average p54Fail p54Succ}}%){{else}}{{#dalbit_if p54Fail '!=' 0}}<span class="font-bold">{{addComma p54Succ}}</span><br/>({{addComma p54Fail}})<br/>({{average p54Fail p54Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="55" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p55Succ '!=' 0}}<span class="font-bold">{{addComma p55Succ}}</span><br/>({{addComma p55Fail}})<br/>({{average p55Fail p55Succ}}%){{else}}{{#dalbit_if p55Fail '!=' 0}}<span class="font-bold">{{addComma p55Succ}}</span><br/>({{addComma p55Fail}})<br/>({{average p55Fail p55Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="56" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p56Succ '!=' 0}}<span class="font-bold">{{addComma p56Succ}}</span><br/>({{addComma p56Fail}})<br/>({{average p56Fail p56Succ}}%){{else}}{{#dalbit_if p56Fail '!=' 0}}<span class="font-bold">{{addComma p56Succ}}</span><br/>({{addComma p56Fail}})<br/>({{average p56Fail p56Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="57" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p57Succ '!=' 0}}<span class="font-bold">{{addComma p57Succ}}</span><br/>({{addComma p57Fail}})<br/>({{average p57Fail p57Succ}}%){{else}}{{#dalbit_if p57Fail '!=' 0}}<span class="font-bold">{{addComma p57Succ}}</span><br/>({{addComma p57Fail}})<br/>({{average p57Fail p57Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="58" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p58Succ '!=' 0}}<span class="font-bold">{{addComma p58Succ}}</span><br/>({{addComma p58Fail}})<br/>({{average p58Fail p58Succ}}%){{else}}{{#dalbit_if p58Fail '!=' 0}}<span class="font-bold">{{addComma p58Succ}}</span><br/>({{addComma p58Fail}})<br/>({{average p58Fail p58Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="61" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p61Succ '!=' 0}}<span class="font-bold">{{addComma p61Succ}}</span><br/>({{addComma p61Fail}})<br/>({{average p61Fail p61Succ}}%){{else}}{{#dalbit_if p61Fail '!=' 0}}<span class="font-bold">{{addComma p61Succ}}</span><br/>({{addComma p61Fail}})<br/>({{average p61Fail p61Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="92" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p92Succ '!=' 0}}<span class="font-bold">{{addComma p92Succ}}</span><br/>({{addComma p92Fail}})<br/>({{average p92Fail p92Succ}}%){{else}}{{#dalbit_if p92Fail '!=' 0}}<span class="font-bold">{{addComma p92Succ}}</span><br/>({{addComma p92Fail}})<br/>({{average p92Fail p92Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="93" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p93Succ '!=' 0}}<span class="font-bold">{{addComma p93Succ}}</span><br/>({{addComma p93Fail}})<br/>({{average p93Fail p93Succ}}%){{else}}{{#dalbit_if p93Fail '!=' 0}}<span class="font-bold">{{addComma p93Succ}}</span><br/>({{addComma p93Fail}})<br/>({{average p93Fail p93Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="94" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p94Succ '!=' 0}}<span class="font-bold">{{addComma p94Succ}}</span><br/>({{addComma p94Fail}})<br/>({{average p94Fail p94Succ}}%){{else}}{{#dalbit_if p94Fail '!=' 0}}<span class="font-bold">{{addComma p94Succ}}</span><br/>({{addComma p94Fail}})<br/>({{average p94Fail p94Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="95" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p95Succ '!=' 0}}<span class="font-bold">{{addComma p95Succ}}</span><br/>({{addComma p95Fail}})<br/>({{average p95Fail p95Succ}}%){{else}}{{#dalbit_if p95Fail '!=' 0}}<span class="font-bold">{{addComma p95Succ}}</span><br/>({{addComma p95Fail}})<br/>({{average p95Fail p95Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" onmouseover="mouseover($(this));" onmouseout="mouseout($(this));" onclick="yearStateDetailPopup($(this))" data-type="96" data-startdate="{{the_date}}" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if p96Succ '!=' 0}}<span class="font-bold">{{addComma p96Succ}}</span><br/>({{addComma p96Fail}})<br/>({{average p96Fail p96Succ}}%){{else}}{{#dalbit_if p96Fail '!=' 0}}<span class="font-bold">{{addComma p96Succ}}</span><br/>({{addComma p96Fail}})<br/>({{average p96Fail p96Succ}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="_fontColor" data-fontcolor="{{#dalbit_if data.gb '==' 'm'}}blue{{/dalbit_if}}{{#dalbit_if data.gb '==' 'f'}}red{{/dalbit_if}}">{{#dalbit_if totSucc '!=' 0}}<span class="font-bold">{{addComma totSucc}}</span><br/>({{addComma totFail}})<br/>({{average totFail totSucc}}%){{else}}{{#dalbit_if totFail '!=' 0}}<span class="font-bold">{{addComma totSucc}}</span><br/>({{addComma totFail}})<br/>({{average totFail totSucc}}%){{/dalbit_if}}{{/dalbit_if}}</td>
    <td class="detailYearTotal _bgColor font-bold" data-bgcolor="#bfbfbf" data-year="{{data.the_date}}">
        <span style="display: none">{{the_date}}</span>
        <span class="_fontColor" data-fontcolor="#ca6927">{{addComma accumTotSucc}}</span><br/>({{addComma accumTotFail}})<br/>({{average accumTotFail accumTotSucc}}%)
    </td>
    </tr>
